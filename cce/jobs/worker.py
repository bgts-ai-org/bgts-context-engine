...
store) and runs the matching :class:`cce.indexing.indexer.Indexer` method. Two connections are
...
reads them from settings (``CCE_BITBUCKET_*``) at execution time.
"""
...
logger = logging.getLogger("cce.jobs.worker")
...
def execute_job(conn: Any, job: dict[str, Any]) -> dict[str, Any]:
    ...
    from cce.indexing.gitsync import GitCredentials
...
class JobWorkerPool:
    """Pool of daemon threads polling the jobs queue; sized by ``CCE_JOB_WORKERS``."""
    ...
    def start(self) -> None:
        ...
        for i in range(count):
            thread = threading.Thread(
                target=self._run_loop, name=f"cce-job-worker-{i}", daemon=True
            )
            thread.start()
    ...
    def _claim_and_run_one(self) -> bool:
        ...
        from cce.jobs.store import claim_next_job, fail_job, finish_job
