...
--
-- Policy model: a session sets ``SET LOCAL cce.user_id = '<id>'``. When the session is a superuser
-- or ``cce.user_id`` is unset/empty (system/pre-auth calls), all rows are visible; otherwise only
...
CREATE OR REPLACE FUNCTION cce_current_user_id() RETURNS TEXT
LANGUAGE sql STABLE AS $$
    SELECT NULLIF(current_setting('cce.user_id', true), '')
$$;
...
CREATE POLICY repos_scope ON repos
    USING (
        cce_current_user_id() IS NULL
        OR EXISTS (
            SELECT 1 FROM scopes s
            WHERE s.repo_id = repos.repo_id AND s.user_id = cce_current_user_id()
        )
    );
...
CREATE POLICY embeddings_scope ON embeddings
    USING (
        ...
        OR EXISTS (
            SELECT 1 FROM scopes s
            WHERE s.repo_id = embeddings.repo_id AND s.user_id = cce_current_user_id()
        )
    );
