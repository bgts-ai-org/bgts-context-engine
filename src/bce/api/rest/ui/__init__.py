...
- All Cypher/SQL reads live in :mod:`bce.api.rest.ui.queries` (nothing is added to
  ``GraphRepository`` or the tool layers).
- The only integration point is the ``ui_router`` registration in ``bce.api.rest.app``.

To remove the UI layer entirely: delete this package and the clearly-marked
"UI layer" block in ``bce/api/rest/app.py``. No other code depends on it.
"""

from bce.api.rest.ui.router import router as ui_router
